...
async def info(message: types.Message, state: FSMContext):
    ...
    await state.set_state(InfoVDS.name)
...
async def get_info(message: types.Message, state: FSMContext):
    ...
    try:
        vds_data = await get_server_info(server_data['ip'], server_data['username'], server_data['password'], int(server_data['port']))
    except Exception as err:
        logger.exception(err)
        await state.finish()
        return await bot.send_message(message.chat.id, '❌ Не удалось получить информацию о сервере', reply_markup=default_keyboard)
...
def setup(dp: Dispatcher):
    ...
    dp.register_message_handler(get_info, state=InfoVDS.name)
